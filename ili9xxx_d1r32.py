...
class ILI9XXX_8B():    
    GPIO_OUT_REG       = const(0x3FF44004) # GPIO 00-31 output register for Esp32
    ...
    def __init__( self, data_pins, cs_pin: int, dc_pin: int, wr_pin: int, rd_pin: int, rst_pin: int, controller = 'ESP32-D1R32', pixel_format = 16, width = 320, height = 480 ):
        """ Constructor
        Args
        ...
        self.cs_bit = 1 << (cs_pin - 32) # for 32-39 pins use different registers
        ...
        # Pregenerated 256 states of gpio (for 8-bit data bus).
    ...
    @micropython.viper
    def set_data_pins(self, byte:int):
        """ Send one byte to data pins D0, D1,...D7
        Args
        byte (int): Byte, example 0x4A
        """
        self.db0.value(byte & 1)
        self.db1.value((byte >> 1) & 1)
        self.db2.value((byte >> 2) & 1)
        self.db3.value((byte >> 3) & 1)
        self.db4.value((byte >> 4) & 1)
        self.db5.value((byte >> 5) & 1)
        self.db6.value((byte >> 6) & 1)
        self.db7.value((byte >> 7) & 1)
    ...
    *** Draw area ***
    ...
    def draw_pixel(self, x:int, y:int, color: int):
        ...
        # Column address sending        
        GPIO_OUT[0] = byte2gpio[0x2A] - dc_bit  # DC = 0 - Command mode
        GPIO_OUT_S[0] = wr_bit
        ...
        # Page address sending
        GPIO_OUT[0] = byte2gpio[0x2B] - dc_bit # DC = 0 - Command mode
        GPIO_OUT_S[0] = wr_bit
        ...
        # Memory write for addresses
        GPIO_OUT[0] = byte2gpio[0x2C] - dc_bit # DC = 0 - Command mode
        GPIO_OUT_S[0] = wr_bit
    ...
    def fill_rect(self, x:int, y:int, x_len:int, y_len:int, color:int):
        ...
        byte2gpio = ptr32(self.BYTE2GPIO) # convert bytes to gpio-state    
        #Getting pointers to registers
        GPIO_OUT   = ptr32(GPIO_OUT_REG)  # 0 - 31  pins
        GPIO_OUT_S = ptr32(GPIO_OUT_W1TS_REG) # + bit
        GPIO_OUT1_S = ptr32(GPIO_OUT1_W1TS_REG) # + bit, 32 - 39 pins
        GPIO_OUT1_C = ptr32(GPIO_OUT1_W1TC_REG) # - bit, 32 - 39 pins
    ...
    *** Image area ***
    ...
    def draw_raw_image(self, filename, x: int, y: int, width: int, height: int):
        ...
        if pxlf == 0x55: # 16-bit
            ...
            #Getting pointers to registers
            GPIO_OUT   = ptr32(GPIO_OUT_REG) # 0 - 31  pins
            GPIO_OUT_S = ptr32(GPIO_OUT_W1TS_REG)
    ...
    def _send_bmp_to_display( self, f, frameHeight: int, frameWidth: int, offset: int, rowsize: int ):
        ...
        GPIO_OUT   = ptr32(GPIO_OUT_REG) # Getting pointer to Pin registers
        GPIO_OUT_S = ptr32(GPIO_OUT_W1TS_REG)
        byte2gpio  = ptr32(self.BYTE2GPIO)

        for row in range(frameHeight):
            # Start position of new row in image-file                                    
            if int(f.tell()) != row * rowsize + offset:
                f.seek(row * rowsize + offset)
