...
def touch(x, y):
    ...
    if checked_square < 9:
        ...
        tft.draw_text_fast('Touch on green square ' + str(checked_square + 1), x_text, y_text, text_color, main_bg)
        ...
        print('Replace the code below in ' + rts_name + '.py :')
...
def show_result(x_min, x_max, x_dir, y_min, y_max, y_dir):
    ...
    tft.draw_text_fast('Calibration finished!', x_text, y_text, text_color, main_bg)
    tft.draw_text_fast('Touch to test', x_text, y_text + 20, text_color, main_bg)
    tft.draw_text_fast('Replace the code below', x_text, y_text + 60, text_color, main_bg)
    tft.draw_text_fast('in ' + rts_name +'.py :', x_text, y_text + 80, text_color, main_bg)
    tft.draw_text_fast('self.X_CALIB=[' + str(x_min) + ',' + str(x_max) + ',' + str(x_dir) + ']', x_text, y_text + 100, text_color, main_bg)
    tft.draw_text_fast('self.Y_CALIB=[' + str(y_min) + ',' + str(y_max) + ',' + str(y_dir) + ']', x_text, y_text + 120, text_color, main_bg)
    run_calibration = False
    

tft.set_font(font10)
tft.fill_screen(main_bg)
tft.draw_text_fast('Touch on green square 1', x_text, y_text, text_color, main_bg)
